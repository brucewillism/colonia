...
if (empty($matricula) || (empty($nome) || empty($endereco) || empty($bairro)|| empty($estado) || empty($cpf) || empty($nascimento) || empty($rgp)|| empty($nome_pai) || empty($nome_mae) || empty($dependente) || empty($data_ins) || empty($insc_inss) ||
...
?>
<?php
$stmt = $conn->prepare("UPDATE pescadores_caicaras SET id = :id, matricula = :matricula, nome = :nome, endereco = :endereco, bairro = :bairro, estado = estado, cpf = :cpf, nascimento = :nascimento, rgp = :rgp, nome_pai = :nome_pai, nome_mae = :nome_mae, dependente = :dependente, data_ins = :data_ins, insc_inss = :insc_inss, rg = :rg, estado_civil = :estado_civil, assinatura_socio = :assinatura_socio, assinatura_presidente = :assinatura_presidente WHERE id = :id");
$stmt->execute(array(
    ':id'=> $id,
    ':matricula'=> $matricula,
    ':nome'=> $nome,
